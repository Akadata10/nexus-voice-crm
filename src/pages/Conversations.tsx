...
export default function Conversations() {
  ...
  const { data: conversations = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const resp = await fetch('/api/telegram/conversations');
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const resp = await fetch(`/api/telegram/messages?conversation_id=${encodeURIComponent(selectedConv.id)}`);
  ...
  // Polling is used via react-query refetchInterval; no realtime subscription required here.

  const initBotMutation = useMutation({
    mutationFn: async () => {
      // Call backend endpoint to initialize bot state (Neon/Postgres)
      const resp = await fetch('/api/telegram/init', {
        method: 'POST',
      ...
      const data = await resp.json();
      if (!resp.ok) throw new Error(data?.error || 'Error initializing bot');
      return data;
    },
    onSuccess: () => {
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      // Call backend endpoint to send via Telegram Bot API (avoids Lovable gateway)
      const resp = await fetch('/api/telegram/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: selectedConv.chat_id, message: replyText.trim() })
      });
      const data = await resp.json();
      if (!resp.ok) throw new Error(data?.error || 'Error sending message');
      return data;
    },
    onSuccess: () => {
      setReplyText("");
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto h-[calc(100vh-8rem)]">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-4">
          ...
          <Button size="sm" onClick={() => initBotMutation.mutate()} disabled={initBotMutation.isLoading || !user} className="bg-primary text-primary-foreground">
            <Power className="w-4 h-4 mr-1" /> {initBotMutation.isLoading ? "..." : "Activar Bot"}
          </Button>
        </motion.div>
            ...
            {selectedConv ? (
              <>
                ...
                >
                  ...
                  />
                  <Button type="submit" size="icon" disabled={sendMutation.isLoading || !replyText.trim()} className="bg-primary text-primary-foreground shrink-0">
