...
async function pushToGithub() {
  ...
  if (!token) {
    ...
    console.error('Configura: set GITHUB_TOKEN=tu_token_aqui');
    process.exit(1);
  }
  ...
  try {
    ...
    for (let i = 0; i < treeItems.length; i++) {
      ...
      if (!blobResponse.ok) {
        throw new Error(`Error creando blob para ${item.path}: ${blobResponse.statusText}`);
      }
    ...
    const treeData = treeItems.map(item => ({
      path: item.path,
      mode: item.mode,
      type: item.type,
    ...
    const commitResponse = await fetch(
      `https://api.github.com/repos/${repo}/git/commits`,
      {
        ...
        body: JSON.stringify({
          message: 'Deploy: Actualizar código del proyecto Nexus Voice CRM',
          tree: tree.sha,
          parents: [baseTreeSha],
          author: {
            name: 'Nexus Bot',
            email: 'bot@nexus-crm.local',
